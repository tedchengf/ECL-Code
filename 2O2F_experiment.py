...
def main():
	...
	OBJ_DICT = {
		2: visual.ImageStim(WIN, "rsc/rc.png"),
		3: visual.ImageStim(WIN, "rsc/rt.png"),
		5: visual.ImageStim(WIN, "rsc/bc.png"),
		7: visual.ImageStim(WIN, "rsc/bt.png"),
	}
	OBJ_LINSPACE = np.arange(-4, 5, 8)
	TRIAL_OBJ_DICT = {
	...
	block_seq = []
	random.shuffle(seq_perms)
	for train_ind in range(3):
		block_seq.append(next(seq_perms[train_ind]))
	repeat_flag = True
	while repeat_flag:
		repeat_flag = test_block(WIN, OBJ_DICT, TRIAL_OBJ_DICT, block_seq, prod_conj, OBJ_LINSPACE, disp_objs)
	core.wait(1)
	...
	# 	random.shuffle(seq_perms)
	# 	block_seq = []
	# 	for seq_gen in seq_perms: block_seq.append(next(seq_gen))
	# 	block_rsp = block(WIN, repeat+1, [block_disp_start, cont_disp0], [block_disp_end, cont_disp1], OBJ_DICT, TRIAL_OBJ_DICT, block_seq, prod_conj, OBJ_LINSPACE, disp_objs)
	# 	with open(DIRECTORY + "Sub_resp.csv", "a") as outfile:
	...
	# 	cont_disp0 = visual.TextBox2(WIN, "Press any key to start",pos = (0, -3), alignment = 'center', letterHeight = 0.8)	
	# 	cont_disp1 = visual.TextBox2(WIN, "Press any key to continue",pos = (0, -3), alignment = 'center', letterHeight = 0.8)	
	# 	block_seq = SIG.fully_permute_sequences()
	# 	block_rsp = block(WIN, repeat+1, [block_disp_start, cont_disp0], [block_disp_end, cont_disp1], OBJ_DICT, TRIAL_OBJ_DICT, block_seq, prod_conj, OBJ_LINSPACE, disp_objs)
	# 	with open(DIRECTORY + "Sub_resp.csv", "a") as outfile:
	# 		for rind in range(len(block_rsp)):
	# 			outfile.write("\t".join(block_rsp[rind].astype(str)))
	...
	GEN_TRIAL_NUM = 6
	SIG_gen = stimuli.Sigma([FILL, SHAPE], ["fill", "shape"], R + 1, generation_mode = GENERATION_MODE)
	sol, non_sol = SIG_gen.satisfies(prod_conj, return_non_solutions = True)
	correct_trials = SIG_gen.fully_permute_sequences(sequences = sol)[:GEN_TRIAL_NUM]
	print(len(sol))
	print(len(non_sol))
	print(len(correct_trials))
	incorrect_trials = SIG_gen.fully_permute_sequences(sequences = non_sol)[:GEN_TRIAL_NUM]
	print(len(incorrect_trials))
	block_seq = correct_trials + incorrect_trials
	print(len(block_seq))
	for repeat in range(1):
		block_disp_start = visual.TextBox2(WIN, "Generalization Block " + str(repeat + 1) + ".", alignment = 'center', letterHeight = 1)
		block_disp_end = visual.TextBox2(WIN, "This is the end of generalization block " + str(repeat + 1) + ".", alignment = 'center', letterHeight = 1)
		cont_disp0 = visual.TextBox2(WIN, "Press any key to start",pos = (0, -3), alignment = 'center', letterHeight = 0.8)	
		cont_disp1 = visual.TextBox2(WIN, "Press any key to continue",pos = (0, -3), alignment = 'center', letterHeight = 0.8)	
		random.shuffle(block_seq)
		block_rsp = block(WIN, "G" + str(repeat+1), [block_disp_start, cont_disp0], [block_disp_end, cont_disp1], OBJ_DICT, TRIAL_OBJ_DICT, block_seq, prod_conj, OBJ_LINSPACE, disp_objs)
		with open(DIRECTORY + "Sub_resp.csv", "a") as outfile:
			for rind in range(len(block_rsp)):
				outfile.write("\t".join(block_rsp[rind].astype(str)))
				outfile.write("\n")	
